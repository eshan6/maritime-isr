...
**Fusion intelligence for the Arabian Sea / Indian west-coast EEZ**
(AOI: 5–25°N, 60–78°E). The platform ingests open sensor feeds, resolves
detections into a persistent object graph of vessels and relationships, and
surfaces dark-vessel and anomalous-behavior alerts with full evidence chains.

This repository contains **two build lineages that share one package**:

| Part | What | Status |
|---|---|---|
| **I. Live-data pipeline** (execution-spec units 0.0–0.5) | Copernicus Sentinel-1 connector, aisstream.io live AIS consumer, NOAA/GFW/registries, SNAP preprocessing, storage on Parquet+DuckDB+R2 | Built; runs on a deploy host with real credentials |
| **II. Synthetic prototype** (roadmap Phases 1–6) | SAR ship detection, AIS track engine, SAR↔AIS fusion + dark-vessel cascade, object graph, anomaly library + risk, analyst product surface | Built; every metric measured on deterministic synthetic data |

> **Honesty note:** Part II numbers are synthetic-suite numbers — proof of
> functionality with injected ground truth, not field accuracy. Part I is the
> path to real data; its connectors are inert without credentials
> (`maritime-isr config` shows what's missing). Nothing here has been measured
> against live feeds yet.

## Quickstart (synthetic prototype — runs anywhere, no credentials)

```bash
pip install -r requirements.txt

# generate synthetic inputs, run all phases, build the product surface
...
python tools/run_phase6_product.py   # -> open data/phase6_product_surface.html

pytest tests/                         # full suite, both lineages
```

## Quickstart (live-data pipeline — needs free accounts)

```bash
pip install -e .
maritime-isr config                   # shows which env vars are missing
maritime-isr ingest s1 --days 7       # Copernicus backfill (CDSE_* env vars)
maritime-isr ingest ais --hours 72    # aisstream.io live consumer
maritime-isr doctor                   # verify SNAP/pyroSAR before preprocessing
```

---

# Part I — Live-data pipeline (execution-spec units 0.0–0.5)
...
# Part II — Synthetic prototype (roadmap Phases 1–6)
